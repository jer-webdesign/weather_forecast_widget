/**
 * Weather Forecast Widget App 
 * ----------------------------------
 * Displays current and 7-day weather forecasts using either mock data or live API data from Open-Meteo.
 *
 * Main Features:
 * - Country and city dropdowns (mock or live)
 * - Temperature unit toggle (°C / °F)
 * - Dark mode toggle
 * - Data source switch (mock or live API)
 *
 * APIs Used:
 ...
 * - Country/City List: https://countriesnow.space/api/v0.1/countries   ( Populates the country and city dropdowns )
 *
 * Mock Mode:
 * - Simulates weather data for Canadian cities
 * - Generates randomized 7-day forecasts for local testing
 *
 * UI Logic:
 * - Dynamically updates DOM when users:
...
// DOM Elements
// Get all HTML elements to interact with
...
function generateMockDaily(city) {
  ...
  return {
    time: [...Array(7)].map((_, i) => new Date(Date.now() + i * 86400000).toISOString().split("T")[0]),
...
async function fetchWeather() {
  ...
  const date = new Date(w.daily.time[1] + "T00:00:00Z");
  ...
  currentContent.innerHTML = `
    ...
    <section class="current-wind-humidity-feels-like">
      ...
      <p><i class="fa-solid fa-temperature-high"></i> Feels Like: ${w.daily.apparent_temperature_max[0]}${getTempUnitSymbol()}</p>
    </section>`;

  // Display 7-day forecast
  forecastGrid.innerHTML = "";
  for (let i = 0; i < 7; i++) {
    const d = new Date(w.daily.time[i] + "T00:00:00Z");
    ...
    card.innerHTML = `
      <section class="weather-forecast">
        ...
        <p>H: ${max}${getTempUnitSymbol()} - L: ${min}${getTempUnitSymbol()}</p> 
